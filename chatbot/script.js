...
const injectChatbotStyles = () => {
  // Aplica estilos de reset específicos para cada classe
  applyChatbotResetStyles();
...
const initChatbot = () => {
  ...
  // Inicializa o gerenciador de exclusão
  const deleteManager = initDeleteManager(chatData);
  const chatDeleteButtons = initChatDeleteButtons(chatData);
  ...
  const switchScreen = (screenToShow) => {
    ...
    // Se estiver mudando para a tela de mensagens, atualiza os botões de exclusão
    if (screenToShow === chatbotElements.screenMessages) {
      chatDeleteButtons.addDeleteButtonsToChats();
    }
  };
  ...
  const openChat = (chatId) => {
    ...
    chat.messages.forEach(message => {
      ...
      messageText.innerHTML = message.text; // Já deve estar formatado no objeto
    ...
    // Atualiza as mensagens com ícones de exclusão
    deleteManager.updateChatMessages(chatId);
  };
  ...
  const createNewChat = () => {
    ...
    // Após adicionar um novo chat ou atualizar a lista, adiciona botões de exclusão
    chatDeleteButtons.addDeleteButtonsToChats();
  };
  ...
  const sendMessage = () => {
    ...
    // Processa o texto da mensagem para garantir formatação adequada
    const formattedMessageText = formatMessageText(messageText);
    ...
    if (messageItem) {
      messageItem.querySelector('.chatbot-message-text').textContent = 
        formatPreviewText(messageText);
      messageItem.querySelector('.chatbot-message-info').textContent = 
        `Você • ${timeString}`;
    }

    // Atualiza as mensagens com ícones de exclusão
    deleteManager.updateChatMessages(chatData.currentChatId);
  
    // Gera resposta baseada em palavras-chave ao invés de usar mensagem genérica
    setTimeout(() => {
      // Usa o sistema de resposta baseado em palavras-chave
      const botResponse = keywordResponder.generateResponse(messageText);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  initChatbot();
  // Inicializa customização de avatar
